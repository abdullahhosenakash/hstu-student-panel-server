...
async function run() {
  try {
    ...
    const studentCollection = client
      .db("HSTUStudentPanel")
      .collection("students");
    const resultCollection_Level_1_Semester_I = client
      .db("studentResults")
      .collection("level-1-semester-I");
    const resultCollection_Level_1_Semester_II = client
      .db("studentResults")
      .collection("level-1-semester-II");
    const resultCollection_Level_2_Semester_I = client
      .db("studentResults")
      .collection("level-2-semester-I");
    const resultCollection_Level_2_Semester_II = client
      .db("studentResults")
      .collection("level-2-semester-II");
    const resultCollection_Level_3_Semester_I = client
      .db("studentResults")
      .collection("level-3-semester-I");
    const resultCollection_Level_3_Semester_II = client
      .db("studentResults")
      .collection("level-3-semester-II");
    const resultCollection_Level_4_Semester_I = client
      .db("studentResults")
      .collection("level-4-semester-I");
    const resultCollection_Level_4_Semester_II = client
      .db("studentResults")
      .collection("level-4-semester-II");
    ...
    app.get("/results/:studentIdLevelSemester", verifyJWT, async (req, res) => {
      ...
      if (level === "1" && semester === "I") {
        result = await resultCollection_Level_1_Semester_I.findOne({
          studentId,
        });
      } else if (level === "1" && semester === "II") {
        result = await resultCollection_Level_1_Semester_II.findOne({
          studentId,
        });
      } else if (level === "2" && semester === "I") {
        result = await resultCollection_Level_2_Semester_I.findOne({
          studentId,
        });
      } else if (level === "2" && semester === "II") {
        result = await resultCollection_Level_2_Semester_II.findOne({
          studentId,
        });
      } else if (level === "3" && semester === "I") {
        result = await resultCollection_Level_3_Semester_I.findOne({
          studentId,
        });
      } else if (level === "3" && semester === "II") {
        result = await resultCollection_Level_3_Semester_II.findOne({
          studentId,
        });
      } else if (level === "4" && semester === "I") {
        result = await resultCollection_Level_4_Semester_I.findOne({
          studentId,
        });
      } else if (level === "4" && semester === "II") {
        result = await resultCollection_Level_4_Semester_II.findOne({
          studentId,
        });
      }
    ...
    app.post("/updateResult", verifyJWT, async (req, res) => {
      ...
      let result;
      if (level === "1" && semester === "I") {
        const isAvailable = await resultCollection_Level_1_Semester_I.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
            message: `The result for ${studentResult.studentName}, ${studentResult.studentId} is already updated!`,
          });
        } else {
          result = await resultCollection_Level_1_Semester_I.insertOne(
            studentResult
          );
        }
      } else if (level === "1" && semester === "II") {
        const isAvailable = await resultCollection_Level_1_Semester_II.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
          ...
          result = await resultCollection_Level_1_Semester_II.insertOne(
            studentResult
          );
        }
      } else if (level === "2" && semester === "I") {
        const isAvailable = await resultCollection_Level_2_Semester_I.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
          ...
          result = await resultCollection_Level_2_Semester_I.insertOne(
            studentResult
          );
        }
      } else if (level === "2" && semester === "II") {
        const isAvailable = await resultCollection_Level_2_Semester_II.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
          ...
          result = await resultCollection_Level_2_Semester_II.insertOne(
            studentResult
          );
        }
      } else if (level === "3" && semester === "I") {
        const isAvailable = await resultCollection_Level_3_Semester_I.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
          ...
          result = await resultCollection_Level_3_Semester_I.insertOne(
            studentResult
          );
        }
      } else if (level === "3" && semester === "II") {
        const isAvailable = await resultCollection_Level_3_Semester_II.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
          ...
          result = await resultCollection_Level_3_Semester_II.insertOne(
            studentResult
          );
        }
      } else if (level === "4" && semester === "I") {
        const isAvailable = await resultCollection_Level_4_Semester_I.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
          ...
          result = await resultCollection_Level_4_Semester_I.insertOne(
            studentResult
          );
        }
      } else if (level === "4" && semester === "II") {
        const isAvailable = await resultCollection_Level_4_Semester_II.findOne({
          studentId: studentResult.studentId,
        });
        if (isAvailable) {
          return res.send({
          ...
          result = await resultCollection_Level_4_Semester_II.insertOne(
            studentResult
          );
        }
      }
